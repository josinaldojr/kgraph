...
TBD - extracted from kgraph-graphify-evolution change. Defines an MCP (Model Context Protocol) server exposed via `kgraph mcp`, letting AI coding assistants query the graph, fetch node details, trace paths, and export the full graph as MCP tools over stdio or HTTP transport.
