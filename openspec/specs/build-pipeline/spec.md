...
TBD - extracted from kgraph-graphify-evolution change. Defines the multi-stage build pipeline (Parse → Enrich → Analyze → Persist) that produces the enriched knowledge graph, and the CLI controls for skipping or independently re-running the Analyze stage.
