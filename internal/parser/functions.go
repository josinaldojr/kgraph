...
// receiver's Struct node (pass 3a). This runs after extractTypes so every
// receiver struct already exists.
...
func (ex *extractor) extractFunc(pkg *packages.Package, fd *ast.FuncDecl) {
	...
	if ex.g.Node(structID) == nil {
		// Receiver type isn't a struct we extracted (e.g. a named
		// non-struct type); skip the has_method edge rather than error.
		return
	}
	edge := &graph.Edge{
