...
// the ORM mapping pass (which needs every struct to be known first, so it
// can resolve foreign-key-shaped fields that reference another struct).
type ormFieldCandidate struct {
	...
	fieldID    string
	fieldName  string
	goTypeName string // base type name of the field, for FK heuristics
	tag        string
}
