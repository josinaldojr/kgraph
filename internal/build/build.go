...
// Run extracts repoPath's full graph and persists it to the database at
// dbPath, recording the current commit as the baseline for future
// `kgraph update` calls (best-effort: repoPath not being a git repo is not
// an error here, since build/context/search don't require git).
func Run(repoPath, dbPath string) (Result, error) {
